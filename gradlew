...
# Add a user-defined classpath with the application
if [ -f "$APP_HOME/CLASSPATH" ]; then
    APP_CLASSPATH=$(<"$APP_HOME/CLASSPATH")
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    ROOTDIRSRAW=$(find -L / -maxdepth 2 -name .gradle -o -name .git 2>/dev/null)
    SEP=""
    for dir in $ROOTDIRSRAW ; do
        ROOTDIRS="$ROOTDIRS$SEP$dir"
        SEP=":"
    done
    OURCYGPATTERN="(^($ROOTDIRS)|.*(^|/)(.gradle|.git)(/|$))"
    OURCYGPATTERN="${OURCYGPATTERN//\/\\/}"
    if [ -z "$GRADLE_CYGPATTERN" ] ; then
